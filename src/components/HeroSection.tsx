...
export default function HeroSection() {
  ...
  return <section className="bg-white text-metal-900 pt-24 pb-16 overflow-hidden">
      ...
      <div className="container-section text-center" id="main-content">
        ...
        duration: 0.4
          ...
          <h1 className={`font-medium tracking-tight ${isMobile ? 'text-2xl sm:text-3xl' : 'text-4xl md:text-5xl lg:text-6xl'} text-metal-900`}>
          ...
          <p className={`text-gray-600 max-w-3xl mx-auto ${isMobile ? 'text-base' : 'text-lg md:text-xl'}`}>Custom Ντουλάπες, lockers &amp; Ντουλάπια, συρταριέρες, αρχειοθήκες πολλαπλών θέσεων &amp; industrial αποθηκευτικές λύσεις, σχεδιασμένες και κατασκευασμένες στην Ελλάδα.</p>
        ...
        delay: 0.2
          ...
          <div ref={carouselRef} className={`md:w-1/2 relative w-full ${isMobile ? 'h-[300px]' : 'h-[350px] sm:h-[450px]'}`} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} role="region" aria-label="Carousel προϊόντων">
            <button onClick={prevImage} className={`absolute left-2 top-1/2 -translate-y-1/2 z-20 bg-white/90 hover:bg-white p-3 rounded-full shadow-md transition-all ${isMobile ? 'w-12 h-12 active:scale-95 active:bg-gray-100' : 'w-10 h-10 hover:scale-105'}`} aria-label="Προηγούμενη εικόνα" style={{
            touchAction: 'manipulation'
          }}>
              <ArrowLeft size={isMobile ? 20 : 16} className="mx-auto" />
            </button>
            
            <button onClick={nextImage} className={`absolute right-2 top-1/2 -translate-y-1/2 z-20 bg-white/90 hover:bg-white p-3 rounded-full shadow-md transition-all ${isMobile ? 'w-12 h-12 active:scale-95 active:bg-gray-100' : 'w-10 h-10 hover:scale-105'}`} aria-label="Επόμενη εικόνα" style={{
            touchAction: 'manipulation'
          }}>
            ...
            {productSlides.map((slide, index) => <div key={index} className={`absolute top-0 left-0 w-full h-full ${currentImage === index ? "opacity-100" : "opacity-0"} transition-opacity duration-1000`}>
                <div className="w-full h-full">
                  <div className="relative w-full h-full">
                    <img src={slide.image} alt={slide.title} width={isMobile ? "300" : "650"} height={isMobile ? "300" : "450"} loading={index < 3 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : index < 3 ? "high" : "low"} decoding={index < 3 ? "sync" : "async"} className="w-full h-full object-contain rounded-2xl" />
                  </div>
                </div>
              </div>)}
            ...
            <div className="absolute -bottom-8 left-0 right-0 flex justify-center gap-2 z-10">
              {productSlides.map((_, index) => <button key={index} className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full transition-all duration-200 ${currentImage === index ? 'bg-blue-600 scale-125' : 'bg-gray-400 hover:bg-gray-500 hover:scale-110'}`} onClick={() => setCurrentImage(index)} aria-label={`Μετάβαση στη διαφάνεια ${index + 1}`} />)}
            </div>
          </div>
          
          <div className="md:w-1/2 text-left space-y-4">
            <h2 className={`font-semibold text-metal-900 ${isMobile ? 'text-xl' : 'text-2xl'}`}>
              {currentSlide.title}
            </h2>
            ...
            <Link to={currentSlide.link}>
              <Button className={`flex items-center gap-2 rounded-xl mt-2 bg-blue-600 hover:bg-blue-700 touch-target ${isMobile ? 'text-sm px-4 py-2' : 'px-6 py-3'}`}>
